...
export default function Reports() {
  ...
  const customerAcquisitionData = (customerData?.customerAcquisition || []).map((entry) => {
    ...
    return {
      ...
      year: year || 0,
      month: month || 0,
      newCustomers: entry.newCustomers || 0
    };
  }).sort((a, b) => (a.year === b.year ? a.month - b.month : a.year - b.year));

  const customerGrowthTotal = customerData?.customerLifetimeValue?.length ||
    customerAcquisitionData.reduce((sum, item) => sum + item.newCustomers, 0);
    ...
    try {
      ...
      switch (reportType) {
        case 'financial': {
          data = {
            ...(financialData || {}),
            paymentAnalysis: salesData?.paymentAnalysis,
  ...
  const generateFinancialPDFContent = (data) => {
    const revenueSummary = data?.revenue || {};
    const totalRevenue = Number(data?.totalRevenue ?? revenueSummary.totalRevenue ?? 0);
    const avgOrderValue = Number(data?.avgOrderValue ?? revenueSummary.avgOrderValue ?? 0);
    const totalOrdersFromSummary = Number(revenueSummary.totalOrders ?? data?.totalOrders ?? 0);
    ...
    const totalOrders = totalOrdersFromSummary || salesTrends.reduce((sum, trend) => sum + (trend.orderCount || 0), 0);
    const paymentLookup = (status) => {
      const match = paymentAnalysis.find(p => (p._id || '').toString().toLowerCase() === status);
      return {
        totalAmount: Number(match?.totalAmount || 0),
        count: Number(match?.count || 0)
      };
    };
    const paidSummary = paymentLookup('paid');
    const pendingSummary = paymentLookup('pending');
    const totalPaymentReceived = paidSummary.totalAmount;
    const pendingPayments = pendingSummary.totalAmount;
                ...
                <div style="background-color: rgb(255, 255, 255); padding: 15px; border: 1px solid rgb(229, 231, 235);">
                  ...
                  ${paymentAnalysis.map(payment => `
                    <div style="margin-bottom: 6px;">
                      <span style="color: rgb(55, 65, 81); font-weight: 500; text-transform: capitalize;">${payment._id || 'status'}:</span>
                      <span style="float: right;">
                        <span style="color: rgb(31, 41, 55); font-weight: bold;">${formatCurrency(payment.totalAmount || 0)}</span>
                        <br><span style="color: rgb(100, 116, 139); font-size: 10px;">${payment.count || 0} orders</span>
                      </span>
                    </div>
                  `).join('')}
                </div>
  ...
  const generateInventoryPDFContent = (data) => {
    const stockLevels = Array.isArray(data?.stockLevels) ? data.stockLevels : [];
    const lowStockItems = Array.isArray(data?.lowStockItems) ? data.lowStockItems : [];
    const stockByCategory = Array.isArray(data?.stockValueByCategory) ? data.stockValueByCategory : [];
    const inventoryValue = data?.inventoryValue || {};
    const materials = Array.isArray(data?.rawMaterials) ? data.rawMaterials : stockLevels;
    const matOrders = Array.isArray(data?.materialOrders) ? data.materialOrders : [];

    const getStockQty = (item = {}) => Number(item.stock_quantity ?? item.current_stock ?? item.current_quantity ?? item.currentQuantity ?? 0);
    const getMinLevel = (item = {}) => Number(item.minimum_stock_level ?? item.reorder_level ?? item.min_level ?? item.minLevel ?? 0);
    const getUnitCost = (item = {}) => Number(item.unit_cost ?? item.unit_price ?? item.unitCost ?? 0);
    const getUnitLabel = (item = {}) => item.unit ?? item.unit_of_measurement ?? item.unitOfMeasurement ?? '';

    const computeTotalValue = (item = {}) => getStockQty(item) * getUnitCost(item);

    const totalStockValue = Number(data?.totalStockValue ?? inventoryValue.totalValue ?? stockLevels.reduce((sum, item) => sum + computeTotalValue(item), 0));
    const totalMaterials = stockLevels.length || stockByCategory.reduce((sum, cat) => sum + Number(cat.itemCount ?? cat.count ?? 0), 0) || materials.length;
    const criticalStockCount = lowStockItems.filter(item => getStockQty(item) <= getMinLevel(item) * 0.5).length;
    const stockTurnoverValue = stockByCategory.reduce((sum, cat) => sum + Number(cat.totalValue || 0), 0);

    const topValueSource = Array.isArray(data?.topValueMaterials) && data.topValueMaterials.length ? data.topValueMaterials : stockLevels;
    const topValueItems = topValueSource
      .map(item => ({
        ...item,
        __computedTotalValue: computeTotalValue(item)
      }))
      .sort((a, b) => b.__computedTotalValue - a.__computedTotalValue);

  return `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #1a1a1a; line-height: 1.3; width: 700px; margin: 0; padding: 0;">
        <!-- Header Section -->
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tbody>
                ${lowStockItems.slice(0, 8).map((item, index) => `
                  <tr style="background: ${index % 2 === 0 ? '#ffffff' : '#fef2f2'}; border-left: 4px solid ${getStockQty(item) <= (getMinLevel(item) * 0.5) ? '#dc2626' : '#f59e0b'};">
                    <td style="padding: 12px; color: #1f2937; font-weight: 600; font-size: 14px; border-bottom: 1px solid #fecaca;">${item.material_name || item.materialName || 'Material'}</td>
                    <td style="padding: 12px; text-align: center; color: #dc2626; font-weight: bold; font-size: 12px; border-bottom: 1px solid #fecaca;">${getStockQty(item)} ${getUnitLabel(item)}</td>
                    <td style="padding: 12px; text-align: center; color: #6b7280; font-size: 12px; border-bottom: 1px solid #fecaca;">${getMinLevel(item)} ${getUnitLabel(item)}</td>
                    <td style="padding: 12px; text-align: right; color: #1f2937; font-weight: 600; font-size: 12px; border-bottom: 1px solid #fecaca;">${formatCurrency(getUnitCost(item))}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
              ...
              <tbody>
                ${materials.slice(0,6).map((m, i) => `
                  <tr style="background: ${i % 2 === 0 ? '#f9fafb' : '#ffffff'};">
                    <td style="padding: 8px; color: #1f2937; font-weight: 600;">${m.material_name || m.materialName || 'Material'}</td>
                    <td style="padding: 8px; text-align: center; color: #374151;">${getStockQty(m)} ${getUnitLabel(m)}</td>
                    <td style="padding: 8px; text-align: right; color: #6b7280;">${formatCurrency(getUnitCost(m))}</td>
                    <td style="padding: 8px; text-align: right; color: #92400e; font-weight: 600;">${formatCurrency(computeTotalValue(m))}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
              ...
              <tbody>
                ${matOrders.slice(0,6).map((o, i) => `
                  <tr style="background: ${i % 2 === 0 ? '#f9fafb' : '#ffffff'};">
                    ...
                    <td style="padding: 8px; text-align: right; color: #059669; font-weight: 600;">${formatCurrency(o.totalCost || o.total_amount || o.total || 0)}</td>
              ...
              <tbody>
                ${stockByCategory.map((category, index) => `
                  <tr style="${index % 2 === 0 ? 'background: #f9fafb;' : 'background: white;'}">
                    ...
                    <td style="padding: 12px 15px; text-align: right; color: #374151; border-bottom: 1px solid #f3f4f6;">${Number(category.itemCount ?? category.count ?? 0)}</td>
                    ...
                    <td style="padding: 12px 15px; text-align: right; color: #6b7280; border-bottom: 1px solid #f3f4f6;">${formatCurrency(category.avgValue ?? safeDivide(category.totalValue || 0, Number(category.itemCount ?? category.count ?? 1)))} </td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
                ...
                ${topValueItems.slice(0, 5).map((material, index) => `
                  <tr style="background: ${index % 2 === 0 ? '#f9fafb' : '#ffffff'};">
                    <td style="padding: 12px; border-bottom: 1px solid #f3f4f6;">
                      <div style="color: #1f2937; font-weight: 600; font-size: 14px;">#${index + 1}. ${material.material_name || material.materialName || 'Material'}</div>
                    </td>
                    <td style="padding: 12px; text-align: center; border-bottom: 1px solid #f3f4f6; color: #6b7280; font-size: 12px;">Stock: ${getStockQty(material)} ${getUnitLabel(material)} | Min: ${getMinLevel(material)} ${getUnitLabel(material)}</td>
                    <td style="padding: 12px; text-align: right; border-bottom: 1px solid #f3f4f6; color: #92400e; font-weight: bold; font-size: 16px;">${formatCurrency(material.__computedTotalValue || computeTotalValue(material))}</td>
                    <td style="padding: 12px; text-align: right; border-bottom: 1px solid #f3f4f6; color: #6b7280; font-size: 11px;">${formatCurrency(getUnitCost(material))} per ${getUnitLabel(material)}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                  <td style="padding: 5px 0; color: #7c2d12;">
                    ${index + 1}. <strong>${item.material_name || item.materialName || 'Material'}</strong> - Order ${Math.max(getMinLevel(item) * 2 - getStockQty(item), 0)} ${getUnitLabel(item)}
                    (Est. Cost: ${formatCurrency(Math.max(getMinLevel(item) * 2 - getStockQty(item), 0) * getUnitCost(item))})
                  </td>
                </tr>
              `).join('')}
            </table>
              ...
              <div className="flex justify-between items-center h-16">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Reports & Analytics</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                  ...
                  <div className="flex items-center gap-4">
                    <Select value={dateRange} onValueChange={setDateRange}>
                      <SelectTrigger className="w-[180px] bg-white/50 backdrop-blur-sm border-white/20">
                    ...
                    <Button onClick={() => fetchReportsData()} variant="outline" className="bg-white/50 backdrop-blur-sm border-white/20">
              ...
              <Card className="relative bg-white/70 backdrop-blur-xl border border-white/20 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 group-hover:scale-105">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <CardTitle className="text-sm font-semibold text-slate-700">Inventory Value</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl font-bold text-slate-800 mb-2">
                ...
                <div className="space-y-4">
                  ...
                  {customerAcquisitionData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={customerAcquisitionData.slice(-6)}>
                        <CartesianGrid strokeDasharray="3 3" />
