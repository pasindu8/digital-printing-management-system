...
export function withAuth(WrappedComponent, allowedRoles = []) {
  return function ProtectedRoute(props) {
    ...
    if (!hasPermission) {
      return <AccessDenied userRole={user?.role} requiredRoute={router.pathname || (typeof window !== 'undefined' ? window.location.pathname : '')} />;
    }
...
export function useRouteProtection() {
  ...
  useEffect(() => {
    const checkRouteAccess = () => {
      const currentPath = router.pathname || (typeof window !== 'undefined' ? window.location.pathname : '');
