...
export function Sidebar({ isOpen, toggleSidebar, isMinimized = false, toggleMinimize }: SidebarProps) {
  ...
  const renderNavItems = (items: SidebarItem[]) => {
    ...
    return filteredItems.map((item) => (
      ...
      >
        <Button
          variant="ghost"
          className={`w-full ${isMinimized ? 'justify-center px-2' : 'justify-start gap-2'} font-normal`}
  ...
  return (
     <div
      className={`fixed top-0 left-0 h-full ${isMinimized ? 'w-16' : 'w-64'} bg-[#eeffe6] shadow-lg z-50
