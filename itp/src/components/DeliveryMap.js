...
const getDeliveryCoordinates = (delivery) => {
  ...
  const lat = parseValue(rawCoordinates.lat ?? rawCoordinates.latitude);
  const lng = parseValue(rawCoordinates.lng ?? rawCoordinates.lon ?? rawCoordinates.longitude);

  if (lat === null || lng === null) {
    return null;
  }

  const inBounds = lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180;
  if (!inBounds) {
    console.warn('Invalid coordinate bounds for delivery', delivery.deliveryId, { lat, lng });
    return null;
  }
...
const DeliveryMap = ({ deliveries, onDeliveryClick }) => {
  ...
  useEffect(() => {
    if (deliveries && deliveries.length > 0) {
      const coordinatesList = deliveries
        .map((delivery) => getDeliveryCoordinates(delivery))
        .filter(Boolean);

      if (coordinatesList.length > 0) {
        const avgLat = coordinatesList.reduce((sum, coords) => sum + coords.lat, 0) / coordinatesList.length;
        const avgLng = coordinatesList.reduce((sum, coords) => sum + coords.lng, 0) / coordinatesList.length;
        setMapCenter([avgLat, avgLng]);
      }
    }
  }, [deliveries]);

  const deliveriesWithCoords = (deliveries || []).reduce((mapped, delivery) => {
  ...
  const mapKey = `map-${deliveriesWithCoords.length}-${deliveriesWithCoords
    .map((delivery) => delivery._id || delivery.deliveryId)
    .join('-')}`;
