...
async function autoPromoteScheduledDeliveries(logContext = 'runtime') {
  try {
    const now = new Date();
    ...
    console.log(`🚚 [${logContext}] Promoting ${deliveriesToUpdate.length} scheduled deliveries to In Transit`);

    await Promise.all(deliveriesToUpdate.map(async (delivery) => {
      delivery.status = 'In Transit';

      // Append tracking update for visibility
      if (Array.isArray(delivery.tracking_updates)) {
        delivery.tracking_updates.push({
          status: 'In Transit',
          timestamp: now,
          location: 'En route',
          notes: 'Automatically marked In Transit based on schedule'
        });
      } else {
      ...
      if (delivery.orderId) {
        try {
          await Order.findByIdAndUpdate(delivery.orderId, { status: 'In_Transit' });
        } catch (orderUpdateErr) {
          console.error('⚠️ Failed to update linked order status for delivery', delivery.deliveryId, orderUpdateErr.message);
        }
      }
    }));
  } catch (err) {
    console.error('❌ Error promoting scheduled deliveries:', err.message);
  }
}

// Run once on startup to align statuses when the service boots
setTimeout(() => {
  autoPromoteScheduledDeliveries('startup').catch((err) => {
    console.error('❌ Startup delivery promotion failed:', err.message);
  });
}, 0);
...
router.get('/', authRequired, async (req, res) => {
  try {
    ...
    await autoPromoteScheduledDeliveries('request:/delivery');
    let deliveries;
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // If delivery status changed to 'Delivered', update order status to 'Completed'
    if (req.body.status === 'Delivered' && updatedDelivery.orderId) {
      await Order.findByIdAndUpdate(updatedDelivery.orderId, { 
        status: 'Completed'
      });
    }
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    // Update related order status when delivery is marked as delivered
...
router.post('/schedule', authRequired, async (req, res) => {
  try {
    ...
    if (!orderId || !driverName || !scheduledDate) {
      return res.status(400).json({ 
        message: 'Missing required fields: orderId, driverName, and scheduledDate are required' 
      });
    }
          ...
          address: (() => {
            ...
            if (order.delivery_address && order.delivery_address.street) {
              return {
                ...
                city: order.delivery_address.city || 'Not specified',
                state: order.delivery_address.state || '',
                zipCode: order.delivery_address.zipCode || ''
              };
            } else if (order.customer_address) {
              // Parse the customer_address string
              const parsed = parseCustomerAddress(order.customer_address);
              return {
                street: parsed.street || 'Address not provided',
