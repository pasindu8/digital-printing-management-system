...
const { Employee: HREmployee, Attendance: HRAttendance, Leave: HRLeave, Salary: HRSalary } = HRModels;
...
const getOrderAmountExpression = () => ({
  $ifNull: ['$final_amount', { $ifNull: ['$total', 0] }]
});

const buildDateMatch = (fieldName, startDate, endDate) => {
  if (!startDate && !endDate) {
    return {};
  }

  const criteria = {};
  if (startDate) criteria.$gte = new Date(startDate);
  if (endDate) criteria.$lte = new Date(endDate);
...
router.get('/dashboard-overview', async (req, res) => {
  try {
    ...
    const orderDateFilter = buildDateMatch('orderDate', startDate, endDate);

    // Revenue and Financial Metrics
    const revenueData = await Order.aggregate([
      { $match: { ...orderDateFilter, payment_status: { $in: ['paid', 'Paid'] } } },
      {
        $group: {
          _id: null,
          totalRevenue: { $sum: getOrderAmountExpression() },
          totalPaidOrders: { $sum: 1 },
          avgOrderValue: { $avg: getOrderAmountExpression() }
        }
      }
    ]);
    ...
    const ordersOverview = await Order.aggregate([
      { $match: Object.keys(orderDateFilter).length ? orderDateFilter : {} },
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
          totalValue: { $sum: getOrderAmountExpression() }
        }
      }
    ]);
  ...
  try {
    ...
    const salesTrends = await Order.aggregate([
      { $match: Object.keys(orderDateFilter).length ? orderDateFilter : {} },
      {
        $group: {
          _id: groupFormat,
          totalSales: { $sum: getOrderAmountExpression() },
          orderCount: { $sum: 1 },
          avgOrderValue: { $avg: getOrderAmountExpression() }
        }
      },
    ...
    const paymentAnalysis = await Order.aggregate([
      { $match: Object.keys(orderDateFilter).length ? orderDateFilter : {} },
      {
        $group: {
          _id: '$payment_status',
          count: { $sum: 1 },
          totalAmount: { $sum: getOrderAmountExpression() }
        }
      }
    ]);
    ...
    const customerLifetimeValue = await Order.aggregate([
      { $match: Object.keys(orderDateFilter).length ? orderDateFilter : {} },
      {
        $group: {
          ...
          customerName: { $first: '$customer_name' },
          totalOrders: { $sum: 1 },
          totalSpent: { $sum: getOrderAmountExpression() },
          avgOrderValue: { $avg: getOrderAmountExpression() },
...
router.get('/hr', async (req, res) => {
  try {
    ...
    const [
      ...
      attendanceTrendRaw,
      employees,
      salaryCollectionStats
    ] = await Promise.all([
      User.aggregate([
        { $match: { role: { $in: ['employee', 'Employee', 'manager', 'Manager', 'admin', 'Admin', 'staff', 'Staff'] } } },
        {
          $group: {
            _id: { $toLower: '$role' },
            count: { $sum: 1 }
      ...
      HRAttendance ? HRAttendance.aggregate([
        {
          $group: {
            _id: { $toLower: '$status' },
            count: { $sum: 1 }
      ...
      HREmployee && HREmployee.find ? HREmployee.find({}, {
        'employment.department': 1,
        'employment.hireDate': 1,
        'employment.salary': 1
      }).lean() : [],
      HRSalary ? HRSalary.aggregate([
        {
          $group: {
            _id: null,
    ...
    const employeeStats = employeeRoleStats.map(stat => ({
      _id: capitalize(stat._id || 'employee'),
      count: stat.count || 0
    }));

    const attendanceData = attendanceStatusStats.map(stat => ({
      _id: normalizeAttendanceStatus(stat._id),
      count: stat.count || 0
    }));

    const leaveData = leaveStatusStats.map(stat => ({
      _id: (stat._id || '').toLowerCase(),
      count: stat.count || 0
    }));
    ...
    employees.forEach(emp => {
      const department = emp?.employment?.department || 'Unassigned';
      ...
      if (emp?.employment?.hireDate) {
        const tenureYears = (Date.now() - new Date(emp.employment.hireDate).getTime()) / MS_PER_YEAR;
        if (isFinite(tenureYears) && tenureYears >= 0) {
          current.totalExperience += tenureYears;
          current.experienceSamples += 1;
        }
      }

      if (typeof emp?.employment?.salary === 'number') {
        current.totalSalary += emp.employment.salary;
        totalSalary += emp.employment.salary;
        salaryCount += 1;
      }
    ...
    const departmentStats = Array.from(departmentMap.values()).map(entry => ({
      department: entry.department,
      employeeCount: entry.employeeCount,
      avgExperience: entry.experienceSamples ? Number((entry.totalExperience / entry.experienceSamples).toFixed(1)) : 0
    }));

    const salaryData = Array.from(departmentMap.values()).map(entry => ({
      _id: entry.department,
      avgSalary: entry.employeeCount ? Number((entry.totalSalary / entry.employeeCount).toFixed(2)) : 0,
      employeeCount: entry.employeeCount,
      totalSalary: Number(entry.totalSalary.toFixed(2))
    }));

    const attendanceTrend = attendanceTrendRaw.map(point => ({
      date: point._id,
    ...
    const totalEmployees = employeeStats.reduce((sum, stat) => sum + (stat.count || 0), 0) || employees.length;
    const totalAttendanceEntries = attendanceData.reduce((sum, stat) => sum + (stat.count || 0), 0);
    const presentCount = attendanceData.find(stat => stat._id === 'present')?.count || 0;
    const attendanceRate = totalEmployees > 0 ? (presentCount / totalEmployees) * 100 : 0;

    const leaveTotals = {
      total: leaveData.reduce((sum, stat) => sum + (stat.count || 0), 0),
      approved: leaveData.find(stat => stat._id === 'approved')?.count || 0,
      pending: leaveData.find(stat => stat._id === 'pending')?.count || 0
    };

    const salarySummary = salaryCollectionStats[0] || { totalNetSalary: 0, totalRecords: 0 };
    const averageSalary = salaryCount > 0 ? totalSalary / salaryCount : 0;

    res.json({
      employeeStats,
      attendanceData,
      attendanceStats: attendanceMonthly,
      attendanceTrend,
      attendanceSummary: {
      ...
      payrollSummary: {
        totalNetSalary: salarySummary.totalNetSalary || 0,
        totalRecords: salarySummary.totalRecords || 0
      }
    });
  } catch (err) {
...
router.get('/financial-summary', async (req, res) => {
  try {
    ...
    const pendingPayments = await Order.aggregate([
      { $match: { ...orderDateFilter, payment_status: { $in: ['pending', 'Pending'] } } },
      {
        $group: {
          _id: null,
          pendingAmount: { $sum: getOrderAmountExpression() },
          pendingOrders: { $sum: 1 }
        }
      }
    ]);
