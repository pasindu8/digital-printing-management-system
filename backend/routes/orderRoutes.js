...
// Multer configuration for temporary file storage before Google Drive upload
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    const uniqueName = `temp_${Date.now()}_${Math.round(Math.random() * 1E9)}${path.extname(file.originalname)}`;
    cb(null, uniqueName);
  }
});
...
const upload = multer({
  storage: storage,
  limits: {
    fileSize: 1 * 1024 * 1024, // 1MB limit as requested
  },
  fileFilter: function (req, file, cb) {
    ...
    if (mimetype && extname) {
      ...
      cb(new Error('Only JPEG, PNG, and PDF files are allowed (max 1MB)'));
    }
  }
});
...
router.post('/', upload.single('design_file'), async (req, res) => {
  try {
    ...
    // Handle design file upload to Google Drive
    if (req.file) {
      try {
        console.log('📤 Uploading design sample to Google Drive...');
        
        const driveFile = await uploadToDrive(
          req.file.path,
          `design_${data.orderId}_${req.file.originalname}`,
          req.file.mimetype,
          'design-samples'
        );
        
        data.design_file_url = driveFile.directLink;
        data.design_file_data = {
        ...
        console.log('✅ Design sample uploaded to Drive successfully');
        
      } catch (error) {
        console.error('❌ Error uploading design sample to Drive:', error);
        // Clean up temp file
        if (req.file && fs.existsSync(req.file.path)) {
          fs.unlinkSync(req.file.path);
        }
        return res.status(500).json({ 
          error: 'Failed to upload design sample to Google Drive',
...
router.get('/ready-for-pickup', authRequired, async (req, res) => {
  try {
    ...
    const transformedOrders = await Promise.all(readyOrders.map(async (order) => {
  const existingDelivery = await Delivery.findOne({ orderId: order._id });
  const baseStatus = order.status || 'Ready_for_Pickup';
  const derivedDeliveryStatus = existingDelivery?.status || null;
  const derivedStatus = existingDelivery ? (derivedDeliveryStatus || 'Scheduled') : baseStatus;
  const derivedScheduledDate = existingDelivery?.scheduledDate || order.expected_completion_date || order.delivery_date;

      return {
        _id: order._id,
        orderId: order.orderId,
        deliveryId: existingDelivery?.deliveryId || `PICKUP-${order.orderId}`,
        customer: {
        ...
        orderStatus: baseStatus,  // Preserve original order status
  deliveryStatus: derivedDeliveryStatus || (existingDelivery ? derivedStatus : null),
  ...
  driverName: existingDelivery?.driverName || null,
...
router.post('/:id/payment-receipt', upload.single('receipt'), async (req, res) => {
  try {
    ...
    let driveFile;
    try {
      console.log('📤 Uploading payment receipt to Google Drive...');
      
      driveFile = await uploadToDrive(
        req.file.path,
        `receipt_${order.orderId}_${req.file.originalname}`,
        req.file.mimetype,
        'receipts'
      );
      
      // Delete old receipt from Drive if exists
      if (order.payment_receipt && order.payment_receipt.driveFileId) {
        try {
          await deleteFromDrive(order.payment_receipt.driveFileId);
          console.log('🗑️ Old receipt deleted from Drive');
        } catch (err) {
          console.error('⚠️ Could not delete old receipt:', err);
        }
      }
      
      // Delete local temp file
      fs.unlinkSync(req.file.path);
      console.log('✅ Receipt uploaded to Drive successfully');
      ...
      return res.status(500).json({ 
        error: 'Failed to upload receipt to Google Drive',
        details: error.message 
      });
    }
